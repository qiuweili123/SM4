...
public class RabbitChannelFactory implements DisposableBean, ShutdownListener {
    ...
    public void shutdownCompleted(ShutdownSignalException cause) {
        if (cause.isInitiatedByApplication()) {
            if (log.isInfoEnabled()) {
                log.info(String.format("Shutdown by application completed for reference [%s] - reason [%s]", cause.getReference(), cause.getReason()));
            }

        } else if (cause.isHardError()) {
            log.error(String.format("Hard error shutdown completed for reference [%s] - reason [%s]", cause.getReference(), cause.getReason()));
        }
        if (log.isInfoEnabled()) {
