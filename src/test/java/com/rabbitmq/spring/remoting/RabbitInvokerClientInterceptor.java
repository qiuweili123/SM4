    ...
    private void createRpcClients() {
        ...
        try {
            ...
            for (int i = 0; i < poolSize; i++) {
                    ...
                    public void handleReturn(int replyCode, String replyText, String exchange, String routingKey, AMQP.BasicProperties properties, byte[] body) throws IOException {
                        ...
                        switch (replyCode) {
                            case AMQP.NO_CONSUMERS:
                                resultException = new UnroutableException(String.format("No consumers for message [%s] - [%s] - [%s]", SerializationUtils.deserialize(body), exchange, routingKey));
                                break;
                            case AMQP.NO_ROUTE:
                                resultException = new UnroutableException(String.format("Unroutable message [%s] - [%s] - [%s]", SerializationUtils.deserialize(body), exchange, routingKey));
                                break;
                            default:
                                resultException = new UnroutableException(String.format("Message returned [%s] - [%s] - [%s] - [%d] - [%s]", SerializationUtils.deserialize(body), exchange, routingKey, replyCode, replyText));
                ...
                log.info(String.format("Started rpc client on exchange [%s(%s)] - routingKey [%s]", exchange, exchangeType, routingKey));
    ...
    public Object invoke(MethodInvocation methodInvocation) throws Throwable {
        ...
        try {
            ...
            if (result.hasInvocationTargetException()) {
                ...
                throw new RemoteInvocationFailureException(String.format("Invocation of method [%s] failed in " + "Rabbit invoker remote service at exchange [%s] - routingKey [%s]", methodInvocation.getMethod(), exchange, routingKey), ex);
            }
        }
    }
