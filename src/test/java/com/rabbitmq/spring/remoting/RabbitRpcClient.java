...
class RabbitRpcClient {
    ...
    public RabbitRpcClient(Channel channel, String exchange, String routingKey, int timeOutMs, boolean mandatory, boolean immediate) throws IOException {
    ...
    public byte[] primitiveCall(AMQP.BasicProperties props, byte[] message) throws IOException, ShutdownSignalException, TimeoutException {
        ...
        synchronized (continuationMap) {
            ...
            if (localProps != null) {
                localProps.builder().correlationId(replyId).replyTo(replyQueue);
            } else {
                localProps = new AMQP.BasicProperties(null, null, null, null, null, replyId, replyQueue, null, null, null, null, null, null, null);
            }
