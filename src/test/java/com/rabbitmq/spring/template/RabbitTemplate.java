...
public class RabbitTemplate implements InitializingBean, ShutdownListener, ReturnListener {
    ...
    public void send(Serializable object, String routingKey, boolean mandatory, boolean immediate) {
        if (log.isTraceEnabled()) {
            log.trace(String.format("Sending object [%s] with routingKey [%s] - mandatory [%s] - immediate [%s]", object, routingKey, mandatory, immediate));
        }
        try {
        ...
        if (channel == null || !channel.isOpen()) {
            try {
                ...
                if (log.isInfoEnabled()) {
                    log.info(String.format("Connected to exchange [%s(%s)] - routingKey [%s]", exchange, exchangeType, routingKey));
                }
            } catch (IOException e) {
    ...
    public void handleBasicReturn(int replyCode, String replyText, String exchange, String routingKey, AMQP.BasicProperties properties, byte[] body) throws IOException {
