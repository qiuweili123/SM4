...
public final class Rijndael_Algorithm // implicit no-argument constructor
{
	...
	static final int debuglevel = DEBUG ? Rijndael_Properties.getLevel(NAME)
			: 0;

	static final PrintWriter err = DEBUG ? Rijndael_Properties.getOutput()
			: null;
	...
	static {
		...
		if (DEBUG && debuglevel > 8) {
			...
			for (i = 0; i < 16; i++) {
				for (j = 0; j < 16; j++)
					System.out
							.print("0x" + byteToString(Si[i * 16 + j]) + ", ");
				System.out.println();
			}
