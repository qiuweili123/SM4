 ...
 * It reads an (algorithm).properties file containing algorithm- specific
 * properties. When using the AES-Kit, this (algorithm).properties file is
 * located in the (algorithm).jar file produced by the "jarit" batch/ script
 * command.
 * <p>
 * 
 * <b>Copyright</b> &copy; 1997, 1998 <a
...
public class Rijndael_Properties // implicit no-argument constructor
{
	...
	private static final String[][] DEFAULT_PROPERTIES = {
			{ "Trace.Rijndael_Algorithm", "true" }, { "Debug.Level.*", "1" },
	...
	static {
		...
		if (!ok) {
			...
			for (int i = 0; i < n; i++)
				properties.put(DEFAULT_PROPERTIES[i][0],
						DEFAULT_PROPERTIES[i][1]);
			if (GLOBAL_DEBUG)
				System.err.println(">>> " + NAME
						+ ": Default properties now set...");
		}
	}
