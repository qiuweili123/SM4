...
public abstract class AbstractDecompressingPostProcessor implements MessagePostProcessor, Ordered {
    ...
    public Message postProcessMessage(Message message) throws AmqpException {
        Object autoDecompress = message.getMessageProperties().getHeaders().get(MessageProperties.SPRING_AUTO_DECOMPRESS);
        if (this.alwaysDecompress || (autoDecompress instanceof Boolean && ((Boolean) autoDecompress))) {
            ...
            try {
                ...
                Assert.state(getEncoding().equals(encoding), "Content encoding must be:" + getEncoding() + ", was:" + encoding);
                if (colonAt < 0) {
