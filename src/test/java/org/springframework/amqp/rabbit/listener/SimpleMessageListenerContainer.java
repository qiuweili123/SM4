...
public class SimpleMessageListenerContainer extends AbstractMessageListenerContainer implements ApplicationEventPublisherAware {
    ...
    public void setConcurrentConsumers(final int concurrentConsumers) {
        ...
        synchronized (consumersMonitor) {
            ...
            if (isActive() && this.consumers != null) {
                if (delta > 0) {
                    Iterator<Entry<BlockingQueueConsumer, Boolean>> entryIterator = consumers.entrySet().iterator();
    ...
    public void setMaxConcurrentConsumers(int maxConcurrentConsumers) {
        ...
        Assert.isTrue(!this.exclusive || maxConcurrentConsumers == 1, "When the consumer is exclusive, the concurrency must be 1");
    ...
    protected void validateConfiguration() {
        ...
        Assert.state(!(getAcknowledgeMode().isAutoAck() && transactionManager != null), "The acknowledgeMode is NONE (autoack in Rabbit terms) which is not consistent with having an " + "external transaction manager. Either use a different AcknowledgeMode or make sure " + "the transactionManager is null.");
        ...
        if (this.getConnectionFactory() instanceof CachingConnectionFactory) {
            ...
            if (cf.getCacheMode() == CacheMode.CHANNEL && cf.getChannelCacheSize() < this.concurrentConsumers) {
                ...
                logger.warn("CachingConnectionFactory's channelCacheSize can not be less than the number " + "of concurrentConsumers so it was reset to match: " + this.concurrentConsumers);
            }
        }
    ...
    protected BlockingQueueConsumer createBlockingQueueConsumer() {
        ...
        consumer = new BlockingQueueConsumer(getConnectionFactory(), this.messagePropertiesConverter, cancellationLock, getAcknowledgeMode(), isChannelTransacted(), actualPrefetchCount, this.defaultRequeueRejected, this.consumerArgs, this.exclusive, queues);
        ...
        try {
            ...
            if (applicationContext != null) {
                ...
                for (Entry<String, Queue> entry : queueBeans.entrySet()) {
                    ...
                    if (queueNames.contains(queue.getName()) && this.rabbitAdmin.getQueueProperties(queue.getName()) == null) {
                        if (logger.isDebugEnabled()) {
                            logger.debug("At least one queue is missing: " + queue.getName() + "; redeclaring context exchanges, queues, bindings.");
                        }
    ...
    private boolean receiveAndExecute(final BlockingQueueConsumer consumer) throws Throwable {
        ...
        if (transactionManager != null) {
            try {
                return new TransactionTemplate(transactionManager, transactionAttribute).execute(new TransactionCallback<Boolean>() {
                    @Override
                    public Boolean doInTransaction(TransactionStatus status) {
                        ConnectionFactoryUtils.bindResourceToTransaction(new RabbitResourceHolder(consumer.getChannel(), false), getConnectionFactory(), true);
                        try {
    ...
    private class AsyncMessageProcessingConsumer implements Runnable {
        ...
        private void logConsumerException(Throwable t) {
            if (logger.isDebugEnabled() || !(t instanceof AmqpConnectException || t instanceof ConsumerCancelledException)) {
                logger.warn("Consumer raised exception, processing can restart if the connection factory supports it", t);
            } else {
                logger.warn("Consumer raised exception, processing can restart if the connection factory supports it. " + "Exception summary: " + t);
            }
        ...
        private void publishEvent(String reason, boolean fatal, Throwable t) {
            if (applicationEventPublisher != null) {
                applicationEventPublisher.publishEvent(new ListenerContainerConsumerFailedEvent(SimpleMessageListenerContainer.this, reason, t, fatal));
            }
        }
