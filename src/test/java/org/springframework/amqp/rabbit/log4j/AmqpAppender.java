        ...
        public void run() {
            try {
                ...
                while (true) {
                    ...
                    @SuppressWarnings("rawtypes") Map props = event.getProperties();
                    @SuppressWarnings("unchecked") Set<Entry<?, ?>> entrySet = props.entrySet();
                    ...
                    try {
                        ...
                        if (retries < maxSenderRetries) {
                            ...
                            errorHandler.error("Could not send log message " + logEvent.getRenderedMessage() + " after " + maxSenderRetries + " retries", e, ErrorCode.WRITE_FAILURE, logEvent);
                        }
                    }
                }
