 ...
 * @author Steve Powell
 */
public class CachingConnectionFactory extends AbstractConnectionFactory implements InitializingBean, ShutdownListener, ApplicationContextAware, ApplicationListener<ContextClosedEvent> {
    ...
    private Channel getChannel(ChannelCachingConnectionProxy connection, boolean transactional) {
        ...
        if (this.cacheMode == CacheMode.CHANNEL) {
            ...
            channelList = transactional ? this.openConnectionTransactionalChannels.get(connection) : this.openConnectionNonTransactionalChannels.get(connection);
        }
        if (channelList == null) {
    ...
    private ChannelProxy getCachedChannelProxy(ChannelCachingConnectionProxy connection, LinkedList<ChannelProxy> channelList, boolean transactional) {
        ...
        return (ChannelProxy) Proxy.newProxyInstance(ChannelProxy.class.getClassLoader(), interfaces, new CachedChannelInvocationHandler(connection, targetChannel, channelList, transactional));
    }
        ...
        synchronized (this.connectionMonitor) {
            if (this.cacheMode == CacheMode.CHANNEL) {
                ...
                if (this.connection == null) {
                    ...
                    System.out.println("##super.createBareConnection()==" + connection);
    ...
    public String toString() {
        return "CachingConnectionFactory [channelCacheSize=" + channelCacheSize + ", host=" + this.getHost() + ", port=" + this.getPort() + ", active=" + active + "]";
    }
    ...
    private class CachedChannelInvocationHandler implements InvocationHandler {
        ...
        public CachedChannelInvocationHandler(ChannelCachingConnectionProxy connection, Channel target, LinkedList<ChannelProxy> channelList, boolean transactional) {
            ...
            if (methodName.equals("equals")) {
                ...
                if (active) {
                    synchronized (this.channelList) {
                        if (!RabbitUtils.isPhysicalCloseRequired() && (this.channelList.size() < getChannelCacheSize() || this.channelList.contains(proxy))) {
        ...
        private void physicalClose() throws Exception {
            ...
            try {
                if (CachingConnectionFactory.this.active && (CachingConnectionFactory.this.publisherConfirms || CachingConnectionFactory.this.publisherReturns)) {
                    ExecutorService executorService = (getExecutorService() != null ? getExecutorService() : CachingConnectionFactory.this.deferredCloseExecutor);
